...
@section('content')
    <x-container>
        ...
        <form method="POST" action="{{ route('documents.download', 'MPOC_Sample.docx') }}" class="space-y-4">
            @csrf
            ...
            <x-card title="Resident Information">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-form.input name="resident_name" label="Resident Name *" placeholder="Enter resident's full name"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-form.input name="requesting_party" label="Resident Party *" placeholder="Enter resident's party"
                        required />

                    <x-form.input name="certification_date" type="date" label="Certification Date" :value="now()->format('Y-m-d')" />
                </div>
            </x-card>
