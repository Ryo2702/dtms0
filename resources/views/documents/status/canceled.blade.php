@extends('layouts.app')

@section('content')
    <div class="container mx-auto max-w-7xl">
        <div class="mb-6">
            <h1 class="text-3xl font-bold">Canceled Documents</h1>
            <p class="text-base-content/70">View and manage documents that have been canceled</p>
        </div>

        @if (session('success'))
            <div class="mb-6 alert alert-success">
                <x-lucide-info class="w-6 h-6" />
                <span>{{ session('success') }}</span>
            </div>
        @endif

        <!-- Reviews Table -->
        <div class="shadow-xl card bg-white-secondary">
            <div class="card-body">
                <h2 class="mb-4 card-title">Canceled Document Reviews</h2>

                <div class="mb-4 alert alert-warning">
                    <x-lucide-x-circle class="w-6 h-6" />
                    <span>
                        These documents have been canceled and are no longer in the active workflow.
                    </span>
                </div>

                <x-data-table 
                    :headers="[
                        'document_id' => 'Document ID',
                        'document_type' => 'Type',
                        'client_name' => 'Client',
                        'status' => 'Status',
                        'created_by' => 'Created By',
                        'canceled_by' => 'Canceled By',
                        'canceled_at' => 'Canceled At',
                        'actions' => 'Actions'
                    ]"
                    :paginator="$canceledReviews"
                    :sortableFields="['document_id', 'document_type', 'client_name', 'status', 'canceled_at']"
                    emptyMessage="No canceled document reviews found."
                >
                    @foreach ($canceledReviews as $review)
                        <tr>
                            <td class="px-4 py-3">
                                <div class="font-mono text-sm">{{ $review->document_id }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="font-semibold">{{ $review->document_type }}</div>
                            </td>
                            <td class="px-4 py-3">{{ $review->client_name }}</td>
                            <td class="px-4 py-3">
                                <span class="badge badge-neutral">
                                    {{ ucfirst($review->status) }}
                                </span>
                            </td>
                            <td class="px-4 py-3">
                                <div>{{ $review->creator->name }}</div>
                                <div class="text-xs text-base-content/70">
                                    {{ $review->creator->department?->name }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <div>{{ $review->reviewer?->name ?? 'N/A' }}</div>
                                <div class="text-xs text-base-content/70">
                                    {{ $review->reviewer?->department?->name ?? '' }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <div>{{ $review->updated_at->format('M d, Y') }}</div>
                                <div class="text-xs text-base-content/70">
                                    {{ $review->updated_at->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex gap-2">
                                    <a href="{{ route('documents.reviews.show', $review->id) }}"
                                        class="btn btn-sm btn-primary">
                                        <x-lucide-eye class="w-4 h-4 mr-1" />
                                        View
                                    </a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
                </x-data-table>
            </div>
        </div>
    </div>
@endsection