        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-4">
            ...
            <div class="form-control">
                ...
                <input type="text" name="name" class="input input-bordered @error('name') input-error @enderror"
                    required value="{{ old('name', $user->name) }}" placeholder="Enter full name">
                @error('name')
                    <label class="label">
            ...
            <div class="form-control">
                ...
                <input type="text" name="municipal_id"
                    ...
                    value="{{ old('municipal_id', $user->municipal_id) }}" placeholder="Enter municipal ID">
                @error('municipal_id')
                    <label class="label">
                        <span class="label-text-alt text-error">{{ $message }}</span>
                    </label>
            ...
            <div class="form-control">
                ...
                <input type="text" name="department"
                    ...
                    value="{{ old('department', $user->department) }}" placeholder="Enter department">
                @error('department')
                    <label class="label">
                        <span class="label-text-alt text-error">{{ $message }}</span>
                    </label>
            ...
            <div class="form-control">
                ...
                <select name="role" class="select select-bordered @error('role') select-error @enderror" required>
                    ...
                    @foreach ($roles as $roleId => $roleName)
                        <option value="{{ $roleId }}"
                            {{ old('role', $user->roles->first()?->id) == $roleId ? 'selected' : '' }}>
                            {{ $roleName }}
                        </option>
                    @endforeach
                </select>
