...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ route('reports.update', $report) }}" method="POST" class="bg-white rounded-lg shadow-md p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $report->name) }}"
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Describe the purpose of this report">{{ old('description', $report->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="report_type" name="report_type"
                    ...
                    <option value="transaction" {{ old('report_type', $report->report_type) === 'transaction' ? 'selected' : '' }}>Transaction Report</option>
                    <option value="workflow" {{ old('report_type', $report->report_type) === 'workflow' ? 'selected' : '' }}>Workflow Report</option>
                    <option value="user" {{ old('report_type', $report->report_type) === 'user' ? 'selected' : '' }}>User Report</option>
                    <option value="department" {{ old('report_type', $report->report_type) === 'department' ? 'selected' : '' }}>Department Report</option>
                </select>
                @error('report_type')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <select id="template_id" name="template_id"
                    ...
                    @foreach ($templates as $template)
                        <option value="{{ $template->id }}" {{ old('template_id', $report->template_id) == $template->id ? 'selected' : '' }}>
                            {{ $template->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <input type="date" id="date_range_start" name="date_range_start" value="{{ old('date_range_start', $report->date_range_start?->format('Y-m-d')) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                ...
                <div>
                    ...
                    <input type="date" id="date_range_end" name="date_range_end" value="{{ old('date_range_end', $report->date_range_end?->format('Y-m-d')) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div class="mb-8">
                <label class="flex items-center">
                    <input type="checkbox" id="is_public" name="is_public" value="1"
                        {{ old('is_public', $report->is_public) ? 'checked' : '' }}
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('reports.show', $report) }}" class="flex-1 px-6 py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors text-center">
                    Cancel
                </a>
            </div>
        </form>
