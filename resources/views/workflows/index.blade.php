                                ...
                                <tbody>
                                    @foreach ($transactionType->workflows as $workflow)
                                        ...
                                        <tr>
                                                        ...
                                                        <div class="group relative">
                                                            <div class="flex items-center">
                                                                <div
                                                                    ...
                                                                    @if ($index === 0)
                                                                        <span class="badge badge-xs badge-info mr-1">Origin</span>
                                                                    @endif
                                                                    <span class="text-xs font-medium">{{ $step['department_name'] ?? 'Unknown' }}</span>
                                                                </div>
                                                            ...
                                                            <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block z-50">
                                                                <div class="bg-neutral text-neutral-content text-xs rounded-lg px-3 py-2 shadow-lg min-w-[180px] max-w-[250px]">
                                                                    <div class="font-semibold mb-1">{{ $step['department_name'] ?? 'Unknown' }}</div>
                                                                    <div class="flex items-center gap-1 text-neutral-content/80">
                                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                                                d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                                        </svg>
                                                                        <span>{{ $step['process_time_value'] ?? 0 }} {{ $step['process_time_unit'] ?? 'days' }}</span>
                                                                    </div>
                                                                    @if (!empty($step['notes']))
                                                                        <div class="mt-1 pt-1 border-t border-neutral-content/20">
                                                                            <div class="text-neutral-content/70 italic">{{ $step['notes'] }}</div>
                                                                        </div>
                                                                    @endif
                                                                    <div class="absolute top-full left-1/2 transform -translate-x-1/2 -mt-1">
                                                                        <div class="border-8 border-transparent border-t-neutral"></div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                            ...
                                            <td>
                                                @if($workflow->documentTags && $workflow->documentTags->count() > 0)
                                                    <div class="flex flex-wrap gap-1 max-w-[200px]">
                                                        @foreach($workflow->documentTags->take(3) as $tag)
                                                            <span class="badge badge-sm {{ $tag->pivot->is_required ? 'badge-primary' : 'badge-ghost' }}" 
                                                                  title="{{ $tag->department->name }}">
                                                        ...
                                                        @if($workflow->documentTags->count() > 3)
                                                            <span class="badge badge-sm badge-ghost">
                                                                +{{ $workflow->documentTags->count() - 3 }} more
                                                            </span>
                                                        @endif
                                                    </div>
                                                @else
                                            ...
                                            <td>
                                                <div class="flex items-center gap-1">
                                                    ...
                                                    <span class="font-medium">{{ $totalTime }} {{ $totalTime == 1 ? 'day' : 'days' }}</span>
                                                </div>
                                            </td>
                                            <td>
